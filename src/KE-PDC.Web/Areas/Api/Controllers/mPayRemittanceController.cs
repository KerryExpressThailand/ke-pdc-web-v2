...
namespace KE_PDC.Web.Areas.Api.Controllers
{
    ...
    [Authorize]
    public class mPayRemittanceController : Controller
    {
        new ApiResponse Response = new ApiResponse();
        private readonly ILogger<mPayRemittanceController> _logger;
        private readonly IHostingEnvironment _hostingEnvironment;
        private KE_POSContext DB;
        private KE_PMGWContext DBPMGW;
        private CultureInfo enUS = new CultureInfo("en-US");
        public mPayRemittanceController(KE_POSContext context, KE_PMGWContext PMGWcontext, ILogger<mPayRemittanceController> logger, IHostingEnvironment hostingEnvironment)
        {
            _logger = logger;
        ...
        public async Task<ActionResult> Get(ReportViewModel Filter, string fileType)
        {
            ...
            try
            {
                ...
                DB.Database.ExecuteSqlCommand(" sp_PDC_mPayRemittanceDate @jsonreq, @jsonOutput OUTPUT ", jsonInput, jsonOutput);
                ResultRemittancedetail MPayRemittance = JsonConvert.DeserializeObject<ResultRemittancedetail> (jsonOutput.Value.ToString());


                List<Remittancedetail> mPay = new List<Remittancedetail>();
                foreach (var item in MPayRemittance.Result)
                {
                    Remittancedetail dc = new Remittancedetail
                    {
                        id = item.id,
                        ...
                        MPay_transaction_Id = item.MPay_transaction_Id,
        ...
        private FileStreamResult ExportExcelLINEPayRemittance(List<Remittancedetail> Remittancedetail)
        {
            // Load the Excel Template
            Stream xlsxStream_mPayRemittancey = System.IO.File.OpenRead(_hostingEnvironment.WebRootPath + @"\assets\templates\mPayRemittance.xlsx");
