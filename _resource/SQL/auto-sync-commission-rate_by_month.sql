...
DECLARE @as_of_month_previous int = 0
	...
	INSERT INTO tb_Commission_Rate (branch_id, as_of_month, vat, commission, dropoff, dhl, cod, insure, bsd, rtsp, psp)
	SELECT	 b.BranchID AS branch_id
			,@as_of_month AS as_of_month
			,ISNULL(cr_previous.vat, 0) AS vat
			,NULL AS commission
			,2 AS dropoff
			,15 AS dhl
			,1 AS cod
	...
	LEFT JOIN tb_Commission_Rate AS cr
		...
		AND cr.as_of_month = @as_of_month
	LEFT JOIN tb_Commission_Rate AS cr_previous
		ON b.BranchID = cr_previous.branch_id
		AND cr_previous.as_of_month = @as_of_month_previous
